...
export default function OptionsPage() {
  ...
  // ── Tab + screener state ─────────────────────────────────────────
  const [tab, setTab] = useState<'builder' | 'screener' | 'coach'>('builder');
  ...
  useEffect(() => {
    if (tab !== 'screener' || screenerLoadedRef.current.has(screenerPreset)) return;
    screenerLoadedRef.current.add(screenerPreset);
    loadScreener(screenerPreset);
  }, [tab, screenerPreset, loadScreener]);

  const handlePresetChange = (preset: string) => {
    setScreenerPreset(preset);
    screenerLoadedRef.current.delete(preset); // force reload on next visit
  };
    ...
    <Layout title="Options">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex gap-0 border-b border-gray-200 dark:border-zinc-800">
          {(['builder', 'screener', 'coach'] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`px-5 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px ${
                tab === t
                  ...
                  : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-zinc-400 dark:hover:text-white'
              }`}
            >
              {t === 'builder' ? '🛠️ Builder' : t === 'screener' ? '🔍 Screener' : '🤖 Options Coach'}
            </button>
          ))}
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm px-5 py-4">
              <div className="flex items-center gap-3 mb-3 flex-wrap">
                ...
                <p className="text-xs text-gray-400 dark:text-zinc-500">IVR · HV20 · GEX regime · Max Pain · Grade — cached 15 min</p>
            ...
            {/* Legend */}
            <div className="flex gap-4 text-xs text-gray-500 dark:text-zinc-400 px-1">
              <span><span className="inline-block w-2 h-2 rounded-full bg-blue-500 mr-1" />Sell Premium (high IVR)</span>
              <span><span className="inline-block w-2 h-2 rounded-full bg-purple-500 mr-1" />Buy Options (low IVR)</span>
              <span><span className="inline-block w-2 h-2 rounded-full bg-gray-400 mr-1" />Neutral</span>
            </div>

            {/* Table */}
            <div className="rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm overflow-x-auto">
              {screenerLoading ? (
                <div className="py-16 text-center text-sm text-gray-400 dark:text-zinc-500 animate-pulse">
                  Fetching options chains… this takes ~10–15s
                </div>
              ) : screenerRows.length === 0 ? (
                <div className="py-16 text-center text-sm text-gray-400 dark:text-zinc-500">
                  No data yet. Select a preset to load.
                </div>
              ) : (
                <table className="w-full text-xs">
                  <thead>
                  ...
                  <tbody>
                    {[...screenerRows]
                      .sort((a, b) => {
                        ...
                        const ga = gradeOrder[a.grade ?? 'null'], gb = gradeOrder[b.grade ?? 'null'];
                        if (ga !== gb) return ga - gb;
                        return (b.ivr ?? 0) - (a.ivr ?? 0);
                      })
                      .map((row) => {
                        const ivrColor =
                          row.ivr === null ? 'text-gray-400' :
                          row.ivr >= 65    ? 'text-red-600 font-bold' :
                          row.ivr <= 30    ? 'text-blue-600 font-bold' : 'text-gray-700 dark:text-zinc-200';
                        const spreadColor =
                          row.ivHvSpread === null ? 'text-gray-400' :
                          row.ivHvSpread > 5 ? 'text-red-600' :
                          row.ivHvSpread < -3 ? 'text-blue-600' : 'text-gray-600 dark:text-zinc-300';
                        const setupBadge =
                          row.setupType === 'sell-premium' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
                          row.setupType === 'buy-options'  ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' :
                          'bg-gray-100 text-gray-500 dark:bg-zinc-800 dark:text-zinc-400';
                        const gradeBadge =
                          row.grade === 'A' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                          row.grade === 'B' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' :
                          'bg-gray-100 text-gray-500 dark:bg-zinc-800 dark:text-zinc-400';
                        return (
                          <tr
                            key={row.symbol}
                            onClick={() => { handleSymbol(row.symbol); setTab('builder'); }}
                          ...
                          >
                            ...
                            <td className="px-3 py-2.5 text-right">
                              {row.gexPositive === null ? <span className="text-gray-400">—</span> : (
                                <span className={row.gexPositive ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold'}>
                                  {row.gexPositive ? '+GEX' : '−GEX'}
                                </span>
                              )}
                            </td>
                            <td className="px-3 py-2.5 text-right text-gray-600 dark:text-zinc-300">
                              {row.maxPainPct !== null ? `${row.maxPainPct > 0 ? '+' : ''}${row.maxPainPct.toFixed(1)}%` : '—'}
                            </td>
                            <td className="px-3 py-2.5">
                              {row.setupType && (
                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${setupBadge}`}>
                                  {row.setupType === 'sell-premium' ? 'Sell' : row.setupType === 'buy-options' ? 'Buy' : 'Neutral'}
                                </span>
                              )}
                            </td>
                            <td className="px-3 py-2.5 text-center">
                              {row.grade && (
                                <span className={`inline-block w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center ${gradeBadge}`}>
                                  {row.grade}
                                </span>
                              )}
                            </td>
            ...
            <p className="text-xs text-gray-400 dark:text-zinc-500 px-1">
              Click any row to load that symbol into the Builder tab. IVR and GEX computed from nearest expiry contracts.
            </p>
          </div>
        )}
        ...
        {tab === 'coach' && (() => {
          const optCtx = hvData ? {
            ...
            hv20: hvData.hv20,
            avgIV: parseFloat(userIV) || (parseFloat(iv) || null),
            ivr: optionsAnalytics?.ivr ?? null,
            ivHvSpread: parseFloat(userIV) && hvData.hv20 > 0 ? parseFloat((parseFloat(userIV) - hvData.hv20).toFixed(1)) : null,
          ...
          void qty; // qty is factored into calcResult.netCost already
          return (
            <OptionsCoachPanel
              symbol={symbol}
