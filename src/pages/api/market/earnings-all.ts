...
// ── Base row (AV CSV only — no enrichment) ────────────────────────────────
interface BaseRow {
  symbol: string;
  name: string;
  reportDate: string;
...
// ── Cache buckets ─────────────────────────────────────────────────────────
const AV_TTL      = 4 * 60 * 60 * 1000;  // 4 hours — AV rate limits
const ENRICH_TTL  = 15 * 60 * 1000;       // 15 min  — matches cron cadence
const VOL_TTL     = 15 * 60 * 1000;       // 15 min  — intraday snapshots
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Fetch all three caches in parallel — each refreshes independently
