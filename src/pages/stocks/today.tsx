...
function ScannerTable({ setups, loading }: { setups: Setup[]; loading: boolean }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden">
      {/* Table header */}
      <div className="hidden md:grid grid-cols-[28px_1fr_120px_90px_90px_90px_60px_100px_44px] gap-3 px-4 py-2.5 bg-gray-50 dark:bg-zinc-800/60 border-b border-gray-200 dark:border-zinc-700 text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-zinc-500">
        <span>#</span>
        <span>Symbol</span>
        <span>Catalyst</span>
        <span>Entry</span>
      ...
      {setups.map((s) => (
        <div key={s.symbol}>
          ...
          <button
            ...
            className="w-full text-left grid grid-cols-1 md:grid-cols-[28px_1fr_120px_90px_90px_90px_60px_100px_44px] gap-3 items-center px-4 py-3.5 border-b border-gray-100 dark:border-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-800/40 transition-colors"
          >
            {/* Rank */}
            <span className="hidden md:block text-xs font-bold text-gray-400 dark:text-zinc-500">{s.rank}</span>
            ...
            <div className="flex items-center gap-3">
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${setupBadgeClass(s.setupType)}`}>{s.setupType}</span>
                </div>
              </div>
            </div>
            ...
            {/* Levels */}
            <span className="hidden md:block text-xs font-mono text-gray-700 dark:text-zinc-200">${fmt(s.entry)}</span>
            <span className="hidden md:block text-xs font-mono text-red-600 dark:text-red-400">${fmt(s.stop)}</span>
            <span className="hidden md:block text-xs font-mono text-emerald-600 dark:text-emerald-400">${fmt(s.target)}</span>

            {/* R:R */}
            <span className="hidden md:block text-xs font-semibold text-gray-700 dark:text-zinc-200">{s.rr}</span>

            {/* Score */}
            <div className="hidden md:block"><ScoreBar score={s.score} /></div>

            {/* Expand arrow */}
            <span className="hidden md:block text-gray-400 dark:text-zinc-500 text-sm">{expanded === s.rank ? '▲' : '▼'}</span>
          </button>

          {/* Expanded detail */}
          {expanded === s.rank && <SetupDetail setup={s} />}
        </div>
      ))}
    </div>
  );
