  ...
  return (
    <ProtectedRoute>
      <div className="flex h-screen bg-background overflow-hidden">
        ...
        {/* Sidebar */}
        <aside
          className={`flex flex-col bg-white dark:bg-brand-navy border-r border-slate-200 dark:border-transparent transition-all duration-300 shrink-0
        ...
        >
          {/* Logo / Brand */}
          <div className={`flex items-center h-16 px-4 border-b border-slate-200 dark:border-white/10 ${collapsed ? 'justify-center' : 'justify-between'}`}>
            {!collapsed ? (
            ...
            <button
              ...
              className={`p-1.5 rounded-lg text-slate-500 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white transition-colors ml-auto`}
            ...
            {NAV_GROUPS.map((group) => {
              ...
              return (
                <div key={group.id} className="mb-1">
                  ...
                  {!collapsed && (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-1.5 rounded-md text-[11px] font-bold uppercase tracking-wider transition-colors ${
                        anyActive ? 'text-brand-green' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                      }`}
                    >
                  ...
                  {!collapsed && <div className="border-t border-slate-200 dark:border-white/10 mx-1 mt-1" />}
                </div>
              );
            })}
          </nav>
          ...
          <div className="border-t border-slate-200 dark:border-white/10 p-2">
            <Link
              href="/profile"
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white transition-colors ${
                router.pathname === '/profile' ? 'bg-slate-100 dark:bg-white/15 text-slate-900 dark:text-white' : ''
              }`}
            ...
            <button
              onClick={onSignOut}
              disabled={signingOut}
              className="mt-1 w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-red-50 dark:hover:bg-red-500/30 hover:text-red-600 dark:hover:text-white transition-colors"
