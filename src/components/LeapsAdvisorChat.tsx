/**
 * LeapsAdvisorChat – the full advisor chat UI without a Layout wrapper.
 * Used as a tab inside the /leaps page.
 */
import React, { useState, useEffect, useRef, useCallback } from 'react';
import type { ChatResponse, LeapsCandidate, ChatIntent } from '@/pages/api/chat/leaps-advisor';
...
const VERDICT_STYLES: Record<string, string> = {
  STRONG_BUY: 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-500/40',
...
// ── Candidates table ──────────────────────────────────────────────────────
function CandidatesTable({ candidates, goalAmount }: { candidates: LeapsCandidate[]; goalAmount: number }) {
  if (!candidates.length) return null;
  return (
    <div className="mt-3 rounded-xl border border-gray-200 dark:border-zinc-700/60 overflow-hidden">
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-2 text-right">
                    ...
                    {c.inTheMoney && <span className="ml-1 text-emerald-600">ITM</span>}
                  </td>
                  <td className="px-3 py-2">
                  ...
                  <td className="px-3 py-2 text-right font-mono">
                    <span className={c.totalCost > 50000 ? 'text-red-400' : c.totalCost > 20000 ? 'text-yellow-400' : 'text-gray-800 dark:text-zinc-200'}>
                      ${c.totalCost.toLocaleString()}
                    </span>
                  </td>
                  <td className="px-3 py-2 text-right font-mono text-gray-500 dark:text-zinc-400">${gainAt2.toLocaleString()}</td>
                  <td className="px-3 py-2 text-right font-mono">
                  ...
                  <td className="px-3 py-2 text-right font-mono text-emerald-600 dark:text-emerald-300">${gainAt5.toLocaleString()}</td>
                  <td className="px-3 py-2 text-right">
                    <span className={c.ivRank < 25 ? 'text-emerald-400' : c.ivRank < 50 ? 'text-yellow-400' : 'text-red-400'}>
                      {c.ivRank.toFixed(0)}
                    </span>
                  </td>
                  <td className="px-3 py-2 text-right">
                    <span className={c.rsi < 35 ? 'text-blue-500 dark:text-blue-400' : c.rsi > 70 ? 'text-red-500 dark:text-red-400' : 'text-gray-700 dark:text-zinc-300'}>
                      {c.rsi.toFixed(0)}
                    </span>
                  </td>
                  <td className="px-3 py-2 text-center"><ScoreBar score={c.aiScore} /></td>
                  <td className="px-3 py-2 text-center"><VerdictBadge verdict={c.aiConsensus} /></td>
      ...
      <div className="flex flex-wrap gap-4 px-4 py-2.5 bg-gray-50 dark:bg-zinc-900/60 border-t border-gray-200 dark:border-zinc-700/30 text-gray-500 dark:text-zinc-600 text-xs">
        ...
        <span>Earnings: days until report (red ≤7d, orange ≤14d)</span>
      </div>
    </div>
  );
}
...
function TypingDots() {
  return (
    <div className="flex items-center gap-1.5 px-4 py-3.5">
      {[0, 1, 2].map((i) => (
        <span key={i} className="w-2 h-2 bg-gray-400 dark:bg-zinc-500 rounded-full animate-bounce" style={{ animationDelay: `${i * 0.18}s` }} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-wrap gap-1.5 mt-2 pl-1">
      {[
        ...
        <span key={label} className="text-xs bg-gray-100 dark:bg-zinc-800/80 text-gray-600 dark:text-zinc-400 rounded-full px-2.5 py-0.5 border border-gray-300 dark:border-zinc-700/50">
          {label}
        </span>
      ))}
...
// ── Chat component ────────────────────────────────────────────────────────
export function LeapsAdvisorChat() {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: `a-${Date.now()}`, role: 'assistant', content: data.message, candidates: data.candidates, intent: data.intent, dataAge: data.dataAge, timestamp: new Date() },
      ]);
    } catch (err: any) {
      setMessages((prev) => [
        ...prev,
        { id: `a-${Date.now()}`, role: 'assistant', content: `Something went wrong: ${err.message ?? 'Unknown error'}. Please try again.`, timestamp: new Date() },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="flex flex-col bg-gray-50 dark:bg-zinc-950 rounded-xl border border-gray-200 dark:border-zinc-800 overflow-hidden" style={{ height: '75vh' }}>
      {/* Header */}
      <div className="flex-shrink-0 border-b border-gray-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/60 backdrop-blur-sm px-5 py-3 flex items-center justify-between">
        <div>
          <h2 className="text-sm font-bold text-gray-900 dark:text-white tracking-tight">LEAPS Advisor</h2>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 space-y-5 min-h-0">
            ...
            <div className={msg.role === 'user' ? 'max-w-lg' : 'max-w-5xl w-full'}>
              <div className={`rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                msg.role === 'user'
                  ...
                  : 'bg-white dark:bg-zinc-800/70 text-gray-800 dark:text-zinc-100 rounded-tl-sm border border-gray-200 dark:border-zinc-700/40 shadow-sm'
              }`}>
                {msg.content.split('\n').map((line, i, arr) => (
                  <span key={i}><MessageContent text={line} />{i < arr.length - 1 && <br />}</span>
                ))}
              </div>
              {msg.candidates && msg.candidates.length > 0 && (
                <CandidatesTable candidates={msg.candidates} goalAmount={msg.intent?.goalAmount ?? 1000} />
              )}
              {msg.intent && msg.candidates && msg.candidates.length > 0 && (
                <>
                  <IntentPills intent={msg.intent} />
                  {msg.dataAge && <p className="text-xs text-gray-400 dark:text-zinc-600 mt-1.5 pl-1">Market data: {msg.dataAge}</p>}
                </>
              )}
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-600 to-violet-700 flex items-center justify-center text-xs font-bold text-white mr-3 flex-shrink-0 shadow-lg">AI</div>
            <div className="bg-white dark:bg-zinc-800/70 border border-gray-200 dark:border-zinc-700/40 rounded-2xl rounded-tl-sm shadow-sm">
              <TypingDots />
            </div>
          </div>
        )}
      ...
      {/* Input area */}
      <div className="flex-shrink-0 border-t border-gray-200 dark:border-zinc-800 bg-white/90 dark:bg-zinc-900/70 backdrop-blur-sm px-4 py-3">
        ...
        <div className="flex gap-2 items-end">
          <textarea
            ...
            placeholder='Try: "Make $2,000 this week with moderate risk"'
            disabled={loading}
            rows={1}
            className="flex-1 bg-white dark:bg-zinc-800/80 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-zinc-600 rounded-xl px-4 py-3 text-sm resize-none outline-none border border-gray-300 dark:border-zinc-700 focus:border-indigo-500 transition-colors disabled:opacity-50"
